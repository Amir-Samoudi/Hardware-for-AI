...
class QuantConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True, 
        ...
        # Weight quantizer using LSQ+ algorithm
        # w_bits: Number of bits for weight quantization (default 8)
        # all_positive: Whether weights are constrained to be positive
        # per_channel: Use channel-wise quantization for weights
        ...
        # Activation quantizer using LSQ+ algorithm
        # a_bits: Number of bits for activation quantization (default 8)
        # all_positive: Activations are expected to be positive (for ReLU-like activations)
    ...
    def forward(self, x):
        # Quantize weights using learned quantizer
        quantized_weight = self.weight_quantizer(self.weight)
        
        # Perform convolution operation with quantized weights
        output = F.conv2d(x, quantized_weight, self.bias, self.stride, self.padding, self.dilation, self.groups)
        
        # Quantize activations (outputs) using activation quantizer
        output = self.activation_quantizer(output) 
        
        return output


# Define a quantized linear layer inheriting from PyTorch's standard Linear layer
...
# Function to prepare a model for quantization by replacing layers
def prepare(model, w_bits=8, a_bits=8):
    def replace_layers(module):
        # Recursively replace layers in the module
        for name, child in module.named_children():
            # Replace Conv2d layers with QuantConv2d
            if isinstance(child, nn.Conv2d):
                ...
                quant_conv = QuantConv2d(
                    ...
                    w_bits=w_bits,      # Weight quantization bits
                    a_bits=a_bits,      # Activation quantization bits
                    per_channel=True,   # Use channel-wise quantization
                    all_positive=False  # Weights can be both positive/negative
                )
                
                # Copy pre-trained weights to quantized layer
            ...
            # Recursively process nested modules (e.g., Sequential blocks)
            else:
                replace_layers(child)
    
    # Start layer replacement from the root module
